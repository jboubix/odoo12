...
class Contract(models.Model):
    ...
    def _compute_desde_sueldo_diario(self):
        if self.sueldo_diario:
            self.sueldo_diario_palabras = amount_to_text().amount_to_text_cheque(self.sueldo_diario, 'pesos', 'M. N.')
            self.bono_asistencia = self.sueldo_diario * 30 * 0.1
            self.bono_puntualidad = self.bono_asistencia
            self.fondo_ahorro = self.sueldo_diario * 30 * 0.033


    @api.one
    @api.depends('total_vales_despensa')
    def _compute_total_vales_despensa(self):
        if self.total_vales_despensa:
            self.total_vales_despensa_palabras = amount_to_text().amount_to_text_cheque(self.total_vales_despensa, 'pesos', 'M. N.')
    ...
    def _compute_ciclo_pago(self):
        if self.periodicidad_pago:
            ...
            self.ciclo_pago = switcher[self.periodicidad_pago]
            if self.ciclo_pago == 'semana':
                self.total_vales_despensa = 180
            elif self.ciclo_pago == 'quincena':
                self.total_vales_despensa = 360
    ...
    sueldo_diario = fields.Monetary('Sueldo diario',
                                    currency_field='company_currency_id',
                                    help="Salario diario en la moneda de la compañía a la que pertenece el empleado.")
    sueldo_hora = fields.Monetary('Sueldo por hora',
                                    currency_field='company_currency_id')
    sueldo_diario_integrado = fields.Monetary('Sueldo diario integrado',
                                    currency_field='company_currency_id')
    sueldo_base_cotizacion = fields.Monetary('Sueldo base cotización',
                                    currency_field='company_currency_id')
    tablas_cfdi_id = fields.Many2one('tablas.cfdi','Tabla CFDI')

    bono_productividad = fields.Monetary('Bono productividad',
                                     currency_field='company_currency_id',
                                     help="Bono de asistencia asignado al empleado.")
    bono_asistencia = fields.Monetary('Bono asistencia',
                                     compute='_compute_desde_sueldo_diario', readonly=True, store=True,
                                     currency_field='company_currency_id',
                                     help="Bono de asistencia asignado al empleado.")
    bono_puntualidad = fields.Monetary('Bono puntualidad',
                                     compute='_compute_desde_sueldo_diario', readonly=True, store=True,
                                     currency_field='company_currency_id',
                                     help="Bono de puntualidad asignado al empleado.")
    fondo_ahorro  = fields.Monetary('Fondo de ahorro',
                                     compute='_compute_desde_sueldo_diario', readonly=True, store=True,
                                     currency_field='company_currency_id',
                                     help="Fondo de ahorro asignado al empleado.")
    ...
    sueldo_diario_palabras = fields.Char(string='- Sueldo Diario en Palabras',
                                    compute='_compute_desde_sueldo_diario', store=True,
                                    help='Certificate of incorporation date text')
    total_vales_despensa = fields.Monetary(string="- Vales de Despensa",
                                     compute='_compute_ciclo_pago', readonly=True, store=True,
                                     currency_field='company_currency_id',
                                     help="Total de Vales de Despensa asignado al empleado.")
    total_vales_despensa_palabras = fields.Char(string='- Total de Vales de Despensa en Palabras',
                                    compute='_compute_total_vales_despensa', store=True,
                                    help='Total de Vales de Despensa en palabras asignado al empleado.')
    ingreso_bruto = fields.Monetary(string="- Ingreso Bruto",
                                     currency_field='company_currency_id',
    ...
    ingreso_neto = fields.Monetary(string="- Ingreso Neto",
                                     currency_field='company_currency_id',
