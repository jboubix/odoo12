...
class ResCompany(models.Model):
    ...
    def _get_date_to_text(self):
        self.dnk_deed_date_text = date_to_words(self.dnk_deed_date)


    #Número de Escritura
    dnk_deed_number = fields.Char(string='- Deed Number', required=True,
                                help='Legal deed number of the company')

    #Fecha de Escritura (Date y Char)
    dnk_deed_date = fields.Date(string='- Deed Date', required=True,
                                help='Certificate of incorporation date')
    dnk_deed_date_text = fields.Char(string='- Deed Date Text',
                                compute='_get_date_to_text', store=True,
                                help='Certificate of incorporation date text')

    #Descripción del Giro
    dnk_line_of_business = fields.Char(string='- Line of Business', required=True,
    ...
    dnk_legal_representative = fields.Many2one('res.partner',
                                domain=[('type', '=', 'contact')],
                                string='- Legal Representative', required=True,
                                help='Legal representative person of the company')
    ...
    dnk_public_notary_state_id = fields.Many2one('res.country.state',
                                string='- State', required=True)
    # Campos del contrato de "AVISO DE PRIVACIDAD"
    dnk_notice_of_privacy_representative = fields.Many2one('res.partner',
                                domain=[('type', '=', 'contact')],
                                string='- Representative', required=True,
                                help='"Notice of Privacy" representative person of the company')
